...
from elasticsearch_dsl.document import Index, Mapping, MetaField, ObjectBase
...
from elasticsearch_dsl.utils import DOC_META_FIELDS, META_FIELDS, merge
from pydantic.typing import update_model_forward_refs

from es_odm.field import get_dsl_field


# copy from elasticsearch_dsl
...
class ESDocumentMeta(type):
    def __new__(cls, name, bases, attrs):
        # DocumentMeta filters attrs in place
        attrs["_doc_type"] = ESDocumentOptions(name, bases, attrs)
        return super(ESDocumentMeta, cls).__new__(cls, name, bases, attrs)


class InnerESDocumentMeta(type):
    def __new__(cls, name, bases, attrs):
        # DocumentMeta filters attrs in place
        # attrs["_doc_type"] = ESDocumentOptions(name, bases, attrs)
        attrs["_doc_type"] = ESDocumentOptions(name, (InnerDoc,), attrs)
        return super(InnerESDocumentMeta, cls).__new__(cls, name, (InnerDoc,), attrs)


class ESDocumentOptions(object):
    def __init__(self, name, bases, attrs):
        meta = attrs.pop("Meta", None)

        for k, v in attrs.get('__fields__', {}).items():
            dsl_field = get_dsl_field(v)
            attrs[k] = dsl_field
...
class ESIndexMeta(ESDocumentMeta):
    ...
    def __new__(cls, name, bases, attrs):
        new_cls = super(ESIndexMeta, cls).__new__(cls, name, (Document,), attrs)
        # new_cls = super(ESIndexMeta, cls).__new__(cls, name, bases, attrs)
        if cls._document_initialized:
            index_opts = attrs.pop("Index", None)
            index = cls.construct_index(index_opts, (Document,))
